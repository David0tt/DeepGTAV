...
void Server::checkRecvMessage() {
	...
	if (items[0].revents & ZMQ_POLLIN) {
		...
		if (d.HasMember("commands")) {
			...
			scenario.createPed(pd["model"].GetFloat(), pd["relativeForward"].GetFloat(), pd["relativeRight"].GetFloat(), pd["heading"].GetFloat(), pd["task"].GetInt(), pd["placeOnGround"].GetBool());
		}
