...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--save_dir', default='F:\\EXPORTDIR\\Testing_VisDrone', help='The directory the generated data is saved to')
    ...
    while True:
        try:
            ...
            if count % 100 == 33:
                for i in [20, 50, 80, 150]:
                    client.sendMessage(CreateVehicle("Scorcher", relativeForward=i, relativeRight=0, color=pickRandomColor(), color2=pickRandomColor(), heading=uniform(-180,180)))
                for i in [30, 100]:
                    client.sendMessage(CreateVehicle("Faggio2", relativeForward=i, relativeRight=0, color=pickRandomColor(), color2=pickRandomColor(), heading=uniform(-180,180)))
                for i in [40, 70]:
                    client.sendMessage(CreateVehicle("Hakuchou", relativeForward=i, relativeRight=0, color=pickRandomColor(), color2=pickRandomColor(), heading=uniform(-180,180)))
